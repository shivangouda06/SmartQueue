...
class _AuthPageState extends State<AuthPage> {
  ...
  Future<void> startOtp() async {
    ...
    try {
      await FirebaseAuth.instance.verifyPhoneNumber(
        ...
        verificationCompleted: (PhoneAuthCredential credential) async {
          try {
            ...
            if (mounted) {
              showMessage(context, 'Could not complete automated sign-in.');
            }
          }
        },
      ...
      if (!mounted) return;
      setState(() => loading = false);
      showMessage(context, 'Unable to send OTP right now. Please try again.');
    }
  }
...
class _OtpPageState extends State<OtpPage> {
  ...
  Future<void> requestOtpAgain() async {
    ...
    try {
      await FirebaseAuth.instance.verifyPhoneNumber(
        ...
        verificationCompleted: (PhoneAuthCredential credential) async {
          try {
            ...
            if (mounted) {
              showMessage(context, 'Unable to complete automatic sign-in.');
            }
          }
        },
        verificationFailed: (FirebaseAuthException e) {
        ...
        codeSent: (String codeSentId, int? resendToken) {
          if (!mounted) return;
          verificationId = codeSentId;
          otp.clear();
          setState(() {
            verifying = false;
            seconds = 30;
          });
          showMessage(context, 'New OTP sent successfully.');
        },
        codeAutoRetrievalTimeout: (String codeSentId) {
          if (mounted) {
            verificationId = codeSentId;
            setState(() => verifying = false);
          }
      ...
      if (!mounted) return;
      setState(() => verifying = false);
      showMessage(context, 'Unable to resend OTP right now. Please try again.');
    }
  }
  ...
  Future<void> verify() async {
    ...
    if (code.length != 6) {
      showMessage(context, 'Enter the 6-digit OTP sent to +91 ${widget.mobile}.');
      return;
    }

    final currentVerificationId = verificationId;

    if (currentVerificationId == null || currentVerificationId.isEmpty) {
      showMessage(context, 'OTP request is still being prepared. Please try again.');
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      setState(() => verifying = false);
      showMessage(context, 'Unable to verify the OTP right now. Please try again.');
    }
  }

  Future<void> resend() async {
    otp.clear();
    await requestOtpAgain();
  }
            ...
            child: Padding(
              ...
              child: Row(
                children: [
                  CircleAvatar(
                    radius: 30,
                    backgroundColor: widget.business.color.withOpacity(.12),
                    child: Icon(
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  Text(
                    'Estimated ${widget.business.peopleWaiting * widget.business.avgMinutes} minutes',
                  ),
                ],
              ),
            ),
...
class _EditProfilePageState extends State<EditProfilePage> {
  ...
  Future<void> pickPhoto() async {
    ...
    try {
      ...
      setState(() => photo = dataUri);
    } catch (_) {
      if (!mounted) return;
      showMessage(context, 'Unable to read the selected image.');
    }
  }
      ...
      body: ListView(
        ...
        children: [
          const _InfoStrip(
            ...
            title: 'Grow with SmartQueue',
            text: 'Let customers join your queue before they arrive.',
          ),
          const SizedBox(height: 18),
          TextField(
            controller: business,
